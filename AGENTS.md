...
- 같은 기간 이벤트별 기준선(개선 후와 같은 조건으로 비교할 값): `CI` PR만 중앙값 106초(105.5), p90 153초(152.9)(n=10, runs 33343958797..35405417634, 1번 규칙의 20회 하한 미달). `CI` push만 중앙값 98초, p90 144초(143.5)(n=30, runs 33339073898..35421775251). `Docker Build & Push` push 실행의 suite 구간(실행 생성 → 마지막 `test / *` 잡 종료) 중앙값 99초(98.5), p90 123초(122.6)(n=32, runs 33358781597..35421775411).
...
- 2026-09 개선: push와 PR의 중복 suite 실행 제거, cert rotation 운영 코드(`drover/services/cert_rotation.py`)의 고정 10초 sleep 제거(로컬 pytest 78초 → 11-18초), 스캔 잡의 docker driver 빌드, 서비스 health-check 2초 interval. 투영치(실측 전)는 혼합 `CI` 기준선 중앙값 100초 → 약 82초이고, p90은 Debian apt 미러 지연 때문에 약 150초로 유지될 수 있다(회귀 아님). 이벤트별 기준선에 대한 투영치는 따로 계산하지 않았다.
- 개선 후 측정 방법: `CI`는 이제 PR과 `workflow_dispatch`에서만 실행되므로 기준선 속도(약 3주에 PR 10건)라면 20회 표본에 약 6주가 걸린다(추정). 개선 후 suite 크리티컬 패스는 (a) `CI` PR 실행의 실행 생성 → 마지막 잡 종료와 (b) `Docker Build & Push` push/tag 실행의 실행 생성 → 마지막 `test / *` 잡 종료로 나눠 재고, 각각 위 이벤트별 기준선(`CI` PR 106/153초, Docker push suite 구간 99/123초)과 비교한다. 혼합 기준선(100/151초)과의 비교는 참고용이다. 20회 이상 쌓이면 이 절에 전후 수치를 추가한다.
